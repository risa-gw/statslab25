...
knitr::opts_chunk$set(echo = TRUE, tidy = TRUE, fig_crop = FALSE)
...
+ Population is everyone who falls into the group of interest for your specific research question. 
...
* The **stand error** of a sample's mean is defined as the **standard deviation** of the sampling distribution of the mean. **Question:** What is standard error telling you? What does it mean if you have a high standard error? **Answer:** It's telling you how different your sample mean is likely to be from the population's mean. A high standard error means your sample mean is likely to be farther from the population mean. This means that your sample isn't as good a representation of the population, either because there's high variability in population or your sample size is small. 
...
# Part 2 Using Tidyverse for Plotting (`ggplot2`)
...
```
  ...
  (2) Use this population and a `for`-loop, get a sampling distribution of the sample mean by sampling 100 observations without replacement from the population 1,000 times.  
...
mean_container <- rep(NA, 1000)

# sample 100 obseervations
for (i in 1:1000){
  
  # randomly sample 100 observations
  sample <- sample(pop_norm, size = 100, replace = FALSE)
...
```{r}
# calculating stats for sampling distribution of sample means 
mean = mean(mean_container)
sd_pos = mean + sd(mean_container)
sd_neg = mean - sd(mean_container)
...
0. Load in the LGBTQ movies dataset again. I'm also going to create a column for the release year, since we haven't covered dealing with date data in R yet. 

```{r}
# loading in data
lgbtq_raw <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/main/data/2024/2024-06-25/lgbtq_movies.csv') %>%
  janitor::clean_names()

# creating a date variable
lgbtq_movies <- lgbtq_raw %>% 
  mutate(
    release_year = release_date %>% # creating new variable called release_year
      as.Date() %>% # telling R to treat release_date as a date rather than a numeric variable
      format(., "%Y") # telling R to use just the year from this date variable
  )
```

1. Create a visualization that shows the average rating (`vote_average`) of LGBTQ movies in each year. Feel free to choose the type of plot that you think fits this data best. *Hint*: when you're making visualizations, it often is helpful to start by making a data set with the data you want to plot.

```{r part4-q2}
# creating dataset of average rating in each year
summary <- lgbtq_movies %>% 
  filter(vote_count > 0 & # filtering out observations where no one rated it
           is.na(release_date) == FALSE) %>% # filtering out observations with no release date
...
summary %>%
  ggplot(aes(x = release_year %>% as.integer(), # plotting years as integers so that we can modify the tick labels on the axis
             y = mean_rating)) +
  geom_point(color = "darkblue") +
  labs(title = "Average Rating of LGBTQ Movies by Release Year", 
       x = "Release Year", 
       y = "Average Rating (1-10 Scale)") +
  scale_x_continuous(
    limits = c(1880, 2020),       # enforce range
    breaks = seq(1880, 2020, 10)  # label by decade
  ) +
  scale_y_continuous(
    limits = c(0, 10),       # enforce range
  )
```
