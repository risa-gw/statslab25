...
- **Final Project** 
    ...
    - Intro to [Overleaf](https://www.overleaf.com/read/fwnymhcgwthf#8f643a) (+ RMarkdown template)
...
6. (B) Alternatively, you can calculate the $p$-value of your observed *t* statistic
    ...
    - $p$-value is the probability that the test statistic equals to (or is more extreme than) what we observed  
        ...
        + To get this probability, we sum up the area under your test statistic distribution that falls to the right of your observed point estimate on the higher side and to the left of it on the lower side. By hand you would do this by integrating the probability density function of your test statistic over the area between negative or positive infinity and your obsreved point estimate, but R has a convenient pt() function that does this calculation for you. 
...
```{r}
 2*pt(q = 2.462177, df = 26,lower.tail = FALSE)
```

  (5) Your conclusion of the significance test  

  
We would be unlikely to observe a sample mean at least as extreme as the one we observe if the true population mean was 0. Therefore we can reject the null hypothesis.

## 2.2 One-sample t-test using R

* **Question:** When do you use one-sample t-test? Comparing a sample mean to a hypothesized or known value. Ex. are children's heights in an elementary school class about average for the nation?
...
We test $H_0: \mu_{\text{change}} = 0$ (the mean weight change is 0) against $H_{\text{a1}}: \mu_{\text{change}} \neq 0$ (the mean weight change is not 0, a two-tailed test) or against 
$H_{\text{a2}}: \mu_{\text{change}} > 0$ (the mean weight change is larger than 0, a one-tailed test), using the following code:
...
t.test(weight_df$change, 
       ...
       conf.level = 0.95)
```
Fail to reject the hypothesis that the true change is 4. I.e., we cannot definitively say that the claim that the diets will lead to an increase of 4 pounds is false.
...
```{r part2-exercise}
## filter data for each diet
weight_b <- weight_df %>% filter(diet == "b")
weight_c <- weight_df %>% filter(diet == "c")
